...
def create_cluster(region_name, cluster_name='Airflow-' + str(datetime.now()), release_label='emr-5.21.0',master_instance_type='m3.xlarge', num_core_nodes=2, core_node_instance_type='m3.2xlarge'):
    ...
    cluster_response = emr.run_job_flow(
        ...
        Configurations=[{'Classification': 'livy-conf','Properties': {'livy.server.session.timeout':'5h'}}]
    )
...
# Python(kind=pyspark), R(kind=sparkr) and SQL(kind=sql) and Scala(kind='spark') spark sessions can also be created by changing the value of kind.
